...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        xw = np.dot(X, self.params['W'])
        forward_output = np.add(xw, self.params['b'])
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    moduleNameKey = module_name + '_' + key
                    momentum[moduleNameKey] = _alpha * momentum[moduleNameKey] - _learning_rate * g
                    module.params[key] += momentum[moduleNameKey]
...
def main(main_params, optimization_type="minibatch_sgd"):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # Do not modify them.
            ######################################################################################
            gradient_d1 = model['L2'].backward(d1, grad_a2)
            gradient_h1 = model['drop1'].backward(h1, gradient_d1)
            grad_a1 = model['nonlinear1'].backward(a1, gradient_h1)
            ######################################################################################
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ######################################################################################
            l1_forward = model['L1'].forward(x)
            activation_forward = model['nonlinear1'].forward(l1_forward)
            dropout_forward = model['drop1'].forward(activation_forward, is_train=False)
            a2 = model['L2'].forward(dropout_forward)
            ######################################################################################
            # NOTE: DO NOT MODIFY CODE BELOW THIS, until next TODO
            ######################################################################################
