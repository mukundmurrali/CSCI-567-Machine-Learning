...
class KNN:
    ...
    def predict(self, features):
        ...
        for feature in features:
            neighbourLabels = self.get_k_neighbors(feature)
            finalLabel,_  = Counter(neighbourLabels).most_common(1)[0]
            reslabels.append(finalLabel)
        return reslabels
    ...
    def get_k_neighbors(self, point):
        ...
        resultLabels = []
        distances = []
        for singleRow in self.features:
            distances.append(self.distance_function(singleRow,point))

        indexes = sorted(range(len(distances)), key=lambda i: distances[i])[:self.k]

        for index in indexes:
            resultLabels.append(self.labels[index])
